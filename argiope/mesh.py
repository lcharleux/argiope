    ...
    * :func:`Mesh.set_fields`
    ...
    def __repr__(self):
        ...
        if self.elements is not None:
            Ne = self.elements.index.size
        if self.nodes is not None:
            Nn = self.nodes.index.size
    ...
    def set_elements(self, elabels=None,
        ...
        if elabels is None:
            ...
            # Sets
            for k, v in esets.items():
                self.elements[("sets", k, "")] = v
            ...
            try:
                ...
                for om in range(len(output_maps)):
                    ...
                    columns = pd.MultiIndex.from_product([(om,), np.arange(oshape)],
                                                         names=[into, "vertex"])
                    data = (conn.values[:, output_maps[om]
    ...
    def angles(self, zfill=3):
        ...
        for etype in etypes:
            ...
            df = pd.concat([angles_df, deviation_df],
            ...
            df["stats", "max_abs_angular_deviation"] = abs(
                df.deviation).max(axis=1)
...
def read_msh(path):
    ...
    amesh = Mesh(nlabels = nodes_labels,
...
def write_xdmf(mesh, path, dataformat="XML"):
    ...
    attribute_pattern = Template(
        open(MODPATH + "/templates/mesh/xdmf_attribute.xdmf").read())
    # MAPPINGS
    cell_map = {
        "tri3":   4,
        "quad4":  5,
    ...
    for field in fields:
        ...
        field_data[tag]["FIELD_DIMENSION"] = " ".join(
            [str(l) for l in field.data.shape])
    ...
    if dataformat == "XML":
        ...
        elements_string = 11*" " + \
            "{0}.h5:/CONNECTIVITY/block0_values".format(path)
  ...
  """
    fields_string = "\n".join([attribute_pattern.substitute(
        **value) for key, value in field_data.items()])
    ...
    if "surfaces" in mesh.elements.keys():
        ...
        for sindex in np.unique(sk.codes[0]):
            ...
            if surface.values.sum() != 0:
                ...
                surf_output.append(
                    "*SURFACE, TYPE=ELEMENT, NAME={0}".format(slabel))
                for findex in surface.keys():
                    if surface[findex].sum() != 0:
...
def _make_conn(shape):
    ...
    if len(shape) == 3:
        ...
        counter = 0
        pattern = np.array(
            [0, 1, 1+nx, nx, nx*ny, 1+nx*ny, 1+(nx+1)*ny, (nx+1)*ny])
...
def structured_mesh(shape=(2, 2, 2), dim=(1., 1., 1.)):
    ...
    mesh = Mesh(nlabels=nindex,
                ...
                conn=conn,)
    if len(shape) == 2:
        mesh.elements[("type", "argiope")] = "quad4"
    if len(shape) == 3:
        mesh.elements[("type", "argiope")] = "hexa8"
    return mesh
